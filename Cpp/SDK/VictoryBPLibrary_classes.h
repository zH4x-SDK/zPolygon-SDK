...
public:
	...
	bool STATIC_LineToLineIntersection(struct FVector* IntersectionPoint, const struct FVector& LinePoint1, const struct FVector& LineDir1, const struct FVector& LinePoint2, const struct FVector& LineDir2);
	...
	float STATIC_ConvertPhysicsLinearVelocity(const struct FVector& Velocity, TEnumAsByte<VictoryBPLibrary_ESpeedUnit> SpeedUnit);
	...
	bool STATIC_ClosestPointsOnTwoLines(struct FVector* closestPointLine1, struct FVector* closestPointLine2, const struct FVector& LinePoint1, const struct FVector& lineVec1, const struct FVector& LinePoint2, const struct FVector& lineVec2);
...
public:
	...
	void STATIC_WidgetGetChildrenOfClass(class UWidget* ParentWidget, TArray<class UUserWidget*>* ChildWidgets, class UClass* WidgetClass, bool bImmediateOnly);
	void STATIC_Visibility__GetRenderedActors(class UObject* WorldContextObject, TArray<class AActor*>* CurrentlyRenderedActors, float MinRecentTime);
	void STATIC_Visibility__GetNotRenderedActors(class UObject* WorldContextObject, TArray<class AActor*>* CurrentlyNotRenderedActors, float MinRecentTime);
	...
	void STATIC_VictorySimulateKeyPress(class APlayerController* ThePC, const struct FKey& Key, TEnumAsByte<Engine_EInputEvent> EventType);
	...
	void STATIC_VictoryISM_ConvertToVictoryISMActors(class UObject* WorldContextObject, class UClass* ActorClass, TArray<class AVictoryISM*>* CreatedISMActors, bool DestroyOriginalActors, int MinCountToCreateISM);
	bool STATIC_VictoryIsApplicationRunning(int ProcessId);
	void STATIC_VictoryIntPlusEquals(int* Int, int Add, int* IntOut);
	void STATIC_VictoryIntMinusEquals(int* Int, int Sub, int* IntOut);
	...
	void STATIC_VictoryFloatPlusEquals(float* Float, float Add, float* FloatOut);
	void STATIC_VictoryFloatMinusEquals(float* Float, float Sub, float* FloatOut);
	bool STATIC_VictoryDestructible_DestroyChunk(class UDestructibleComponent* DestructibleComp, int HitItem);
	void STATIC_VictoryCreateProc(int* ProcessId, const struct FString& FullPathOfProgramToRun, TArray<struct FString> CommandlineArgs, bool Detach, bool Hidden, int Priority, const struct FString& OptionalWorkingDirectory);
	void STATIC_VictoryAppendInline(struct FString* String, const struct FString& ToAppend, struct FString* Result, bool AppendNewline);
	...
	class UTexture2D* STATIC_Victory_LoadTexture2D_FromFile_Pixels(const struct FString& FullFilePath, VictoryBPLibrary_EJoyImageFormats ImageFormat, bool* IsValid, int* Width, int* Height, TArray<struct FLinearColor>* OutPixels);
	class UTexture2D* STATIC_Victory_LoadTexture2D_FromFile(const struct FString& FullFilePath, VictoryBPLibrary_EJoyImageFormats ImageFormat, bool* IsValid, int* Width, int* Height);
	...
	TEnumAsByte<AIModule_EPathFollowingRequestResult> STATIC_Victory_AI_MoveToWithFilter(class APawn* Pawn, const struct FVector& Dest, class UClass* FilterClass, float AcceptanceRadius, bool bProjectDestinationToNavigation, bool bStopOnOverlap, bool bCanStrafe);
	...
	bool STATIC_TraceData__GetTraceDataFromSkeletalMeshSocket(struct FVector* TraceStart, struct FVector* TraceEnd, class USkeletalMeshComponent* Mesh, const struct FRotator& TraceRotation, float TraceLength, const struct FName& Socket, bool DrawTraceData, const struct FLinearColor& TraceDataColor, float TraceDataThickness);
	bool STATIC_TraceData__GetTraceDataFromCharacterSocket(struct FVector* TraceStart, struct FVector* TraceEnd, class AActor* TheCharacter, const struct FRotator& TraceRotation, float TraceLength, const struct FName& Socket, bool DrawTraceData, const struct FLinearColor& TraceDataColor, float TraceDataThickness);
	...
	void STATIC_StringConversion__GetFloatAsStringWithPrecision(float TheFloat, struct FString* TheString, int Precision, bool IncludeLeadingZero);
	void STATIC_String__ExplodeString(TArray<struct FString>* OutputStrings, const struct FString& InputString, const struct FString& Separator, int Limit, bool bTrimElements);
	...
	void STATIC_ServerTravel(class UObject* WorldContextObject, const struct FString& MapName, bool bSkipNotifyPlayers);
	void STATIC_Selection_SelectionBox(class UObject* WorldContextObject, TArray<class AActor*>* SelectedActors, const struct FVector2D& AnchorPoint, const struct FVector2D& DraggedPoint, class UClass* ClassFilter);
	void STATIC_seedRandWithTime();
	void STATIC_seedRandWithEntropy();
	void STATIC_seedRand(int Seed);
	bool STATIC_ScreenShots_Rename_Move_Most_Recent(struct FString* OriginalFileName, const struct FString& NewName, const struct FString& NewAbsoluteFolderPath, bool HighResolution);
	void STATIC_SaveGameObject_GetMostRecentSaveSlotFileName(struct FString* Filename, bool* bFound);
	...
	class UAudioComponent* STATIC_PlaySoundAttachedFromFile(const struct FString& FilePath, class USceneComponent* AttachToComponent, const struct FName& AttachPointName, const struct FVector& Location, TEnumAsByte<Engine_EAttachLocation> LocationType, bool bStopWhenAttachedToDestroyed, float VolumeMultiplier, float PitchMultiplier, float StartTime, class USoundAttenuation* AttenuationSettings);
	...
	bool STATIC_OptionsMenu__GetDisplayAdapterScreenResolutions(TArray<int>* Widths, TArray<int>* Heights, TArray<int>* RefreshRates, bool IncludeRefreshRates);
	...
	bool STATIC_Mobility__SetSceneCompMobility(class USceneComponent* SceneComp, TEnumAsByte<Engine_EComponentMobility> NewMobility);
	...
	bool STATIC_LoadStringFromFile(struct FString* Result, const struct FString& FullFilePath);
	bool STATIC_LoadStringArrayFromFile(TArray<struct FString>* StringArray, int* ArraySize, const struct FString& FullFilePath, bool ExcludeEmptyLines);
	...
	void STATIC_JoyGraphicsSettings__FullScreen_Set(TEnumAsByte<VictoryBPLibrary_EJoyGraphicsFullScreen> NewSetting);
	TEnumAsByte<VictoryBPLibrary_EJoyGraphicsFullScreen> STATIC_JoyGraphicsSettings__FullScreen_Get();
	bool STATIC_JoyFileIO_GetFilesInRootAndAllSubFolders(TArray<struct FString>* Files, const struct FString& RootFolderFullPath, const struct FString& Ext);
	bool STATIC_JoyFileIO_GetFiles(TArray<struct FString>* Files, const struct FString& RootFolderFullPath, const struct FString& Ext);
	...
	bool STATIC_HasSubstring(const struct FString& SearchIn, const struct FString& SubString, TEnumAsByte<CoreUObject_ESearchCase> SearchCase, TEnumAsByte<CoreUObject_ESearchDir> SearchDir);
	...
	bool STATIC_GetViewportPositionHitResultByChannel(class UObject* WorldContextObject, const struct FVector2D& ViewportPosition, TEnumAsByte<Engine_ECollisionChannel> TraceChannel, bool bTraceComplex, struct FHitResult* OutHitResult);
	...
	bool STATIC_FileIO__SaveStringTextToFile(const struct FString& SaveDirectory, const struct FString& JoyfulFileName, const struct FString& SaveText, bool AllowOverWriting, bool AllowAppend);
	bool STATIC_FileIO__SaveStringArrayToFile(const struct FString& SaveDirectory, const struct FString& JoyfulFileName, TArray<struct FString> SaveText, bool AllowOverWriting, bool AllowAppend);
	...
	int STATIC_CountOccurrancesOfSubString(const struct FString& Source, const struct FString& SubString, TEnumAsByte<CoreUObject_ESearchCase> SearchCase);
	...
	class AStaticMeshActor* STATIC_Clone__StaticMeshActor(class UObject* WorldContextObject, bool* IsValid, class AStaticMeshActor* ToClone, const struct FVector& LocationOffset, const struct FRotator& RotationOffset);
...
// 0x0230 (FullSize[0x02E0] - InheritedSize[0x00B0])
class UVictoryTMapComp : public UActorComponent
{
public:
	unsigned char                                      UnknownData_WYBY[0x230];                                   // 0x00B0(0x0230) MISSED OFFSET (PADDING)
